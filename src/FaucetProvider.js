...
class FaucetProvider {
  constructor(base, options = {}) {
    ...
    this.useGSN =
      options && typeof options.useGSN !== "undefined" ? options.useGSN : true;
  ...
  _handleGetTransactionReceipt(payload, callback) {
    ...
    this.baseSend(payload, (err, receipt) => {
      if (err) callback(err, null);
        ...
        callback(
          null,
          fixTransactionReceiptResponse(receipt, this.options.verbose)
        );
    });
  ...
  _handleSendTransaction(payload, callback) {
    ...
    if (!txParams.to) {
      return callback(
        new Error("Cannot deploy a new contract via the GSN"),
        null
      );
    }
    if (txParams.value) {
    ...
    callAsJsonRpc(
      this.relayClient.sendTransaction.bind(this.relayClient),
      [payload],
      payload.id,
      callback,
      txHash => {
  ...
  _handleEstimateGas(payload, callback) {
    ...
    if (!this._withGSN(payload, txParams)) return false;
    callAsJsonRpc(
      this.relayClient.estimateGas.bind(this.relayClient),
      [txParams],
      payload.id,
      callback
    );
